...
def load_data():
    ...
    # Exact CLV cannot be calculated from this customer-level dataset because
    # transaction history, lifespan and margin are unavailable.
...
def cluster_label(row, medians):
    ...
    if r <= medians["Recency"] and f >= medians["Frequency"] and m >= medians["Monetary"]:
        return "Loyal High-Value"
    if r <= medians["Recency"] and f < medians["Frequency"] and m >= medians["Monetary"]:
        return "Recent Big Spenders"
    if r > medians["Recency"] and f >= medians["Frequency"] and m >= medians["Monetary"]:
        return "At-Risk High-Value"
    if r > medians["Recency"] and f < medians["Frequency"]:
...
def marketing_action(segment):
    ...
    return actions.get(segment, "Use a personalised campaign based on the segment profile.")
...
if not st.session_state.run_clustering:
    st.info("Select K in the sidebar and click **Run Clustering** to generate the customer segments.")
    st.stop()

k = st.session_state.active_k
clustered, scaler, model, X, silhouette = run_kmeans(df, k)
...
name_map = {
    int(row["Cluster"]): cluster_label(row, medians)
...
profile["Marketing_Action"] = profile["Segment"].map(marketing_action)

# KPI row
c1, c2, c3, c4, c5 = st.columns(5)
c1.metric("Customers", f"{len(clustered):,}")
c2.metric("Avg Purchase Value", f"${clustered['AveragePurchaseValue'].mean():,.2f}")
c3.metric("Avg Frequency", f"{clustered['Frequency'].mean():.2f}")
c4.metric("Avg Monetary", f"${clustered['Monetary'].mean():,.2f}")
c5.metric("Silhouette Score", f"{silhouette:.3f}")
...
with tabs[0]:
    st.subheader("Dataset Inspection & Data Quality")

    a, b, c, d = st.columns(4)
    a.metric("Original Rows", f"{quality['original_rows']:,}")
    b.metric("Final Rows", f"{quality['final_rows']:,}")
    c.metric("Features", quality["features"])
    d.metric("Duplicate Rows Removed", quality["duplicate_rows"])

    q1, q2, q3 = st.columns(3)
    q1.metric("Missing Income Filled", quality["missing_income_filled"])
    q2.metric("Invalid Birth Years Removed", quality["invalid_birth_removed"])
    q3.metric("Invalid Income Rows Removed", quality["invalid_income_removed"])

    st.subheader("Dataset Preview")
    st.dataframe(df.head(10), use_container_width=True)

    st.subheader("RFM & Purchase Descriptive Statistics")
    stats = clustered[[
        "Recency", "Frequency", "Monetary",
        "AveragePurchaseValue", "CLV_Proxy"
    ]].describe().T.round(2)
    ...
    st.warning(
        "CLV note: this dataset is customer-level and does not provide complete transaction history, "
        "customer lifespan or profit margin. Therefore CLV Proxy equals historical Monetary spend; "
        "it is not presented as an exact lifetime-value model."
    )
...
with tabs[1]:
    ...
    for candidate in k_values:
        km = KMeans(n_clusters=candidate, random_state=42, n_init=10)
    ...
    ax.axvline(k, linestyle="--", alpha=0.7, label=f"Selected K = {k}")
    ...
    st.success(
        f"Current clustering uses K = {k}. The silhouette score for this selection is {silhouette:.3f}. "
        "The final K should consider both the elbow point and business interpretability."
    )

with tabs[2]:
    st.subheader("🔵 Customer Cluster Visualisations")
    ...
    counts = (
        clustered.groupby(["Cluster", "Segment"])
        .size()
    ...
    fig, ax = plt.subplots(figsize=(9, 4.5))
    sns.barplot(data=counts, x="Segment", y="Customers", ax=ax)
...
with tabs[3]:
    ...
    display = profile[[
        "Cluster", "Segment", "Customers", "Recency",
        "Frequency", "Monetary", "AveragePurchaseValue",
        "CLV_Proxy", "Marketing_Action"
    ]].copy()

    st.dataframe(display.round(2), use_container_width=True)

    for _, row in profile.sort_values("Customers", ascending=False).iterrows():
        st.markdown(
...
with tabs[4]:
    ...
    st.write(
        "Enter a customer's RFM behaviour. The dashboard standardises the values with "
        "the same transformation used for K-Means and assigns the nearest learned cluster."
    )

    p1, p2, p3 = st.columns(3)
    with p1:
        recency_input = st.number_input("Recency (days)", min_value=0.0, value=30.0, step=1.0)
    with p2:
        frequency_input = st.number_input("Purchase Frequency", min_value=0.0, value=10.0, step=1.0)
    with p3:
        monetary_input = st.number_input("Monetary Value ($)", min_value=0.0, value=500.0, step=50.0)
    ...
    if st.button("🎯 Predict Segment", use_container_width=True):
        ...
        user["Frequency"] = np.log1p(user["Frequency"])
        user["Monetary"] = np.log1p(user["Monetary"])
        user_scaled = scaler.transform(user[["Recency", "Frequency", "Monetary"]])
        predicted_cluster = int(model.predict(user_scaled)[0])
        predicted_segment = name_map[predicted_cluster]

        st.success(
            f"Predicted Segment: **Cluster {predicted_cluster} — {predicted_segment}**"
        )
        st.info(f"Recommended action: {marketing_action(predicted_segment)}")

st.divider()
st.caption("Oasis Infobyte — Data Analytics Task 2 | Customer Segmentation Analysis")
